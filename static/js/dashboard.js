...
function refreshDashboard() {
    ...
    // Clear current stats
    document.getElementById('company-searches').textContent = '-';
    document.getElementById('people-searches').textContent = '-';
    document.getElementById('company-enrichments').textContent = '-';
    document.getElementById('people-enrichments').textContent = '-';
    document.getElementById('success-rate').textContent = '-';
    
    // Load fresh data
    loadDashboardStats().then(() => {
        hideLoading();
        showServerResponseNotification('Dashboard refreshed successfully');
    }).catch(error => {
        hideLoading();
...
// Add the missing resetStats function
function resetStats() {
    if (confirm('Are you sure you want to reset all dashboard statistics? This cannot be undone.')) {
        showLoading();
        makeRequest('/api/dashboard/reset', 'POST')
            .then(response => {
                hideLoading();
                if (response.success) {
                    showServerResponseNotification('Statistics reset successfully');
                    refreshDashboard(); // Refresh to show zeroed stats
                } else {
                    showError('Failed to reset stats: ' + (response.error || 'Unknown error'));
                }
            })
            .catch(error => {
                hideLoading();
                showError('Failed to reset stats: ' + error.message);
            });
    }
}
